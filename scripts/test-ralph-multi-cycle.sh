...
# Regressão do finding 7 (refactor-radar, HND-2026-0011): um bloco com MÚLTIPLOS
# ciclos de correção produz vários *.result.json de impl no mesmo attempt. O
# contrato normativo deve aceitar UMA implementação bem-sucedida (a mais recente
# do ciclo final) + UMA revisão, em vez de exigir exatamente um arquivo por modo
# (que gerava recovery.required em loop).
#
# shellcheck disable=SC2016
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TMP="$(mktemp -d "${TMPDIR:-/tmp}/ralph-method-multicycle.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
...
git -C "$project" config user.name 'Ralph Method Multi Cycle Test'
printf '%s\n' '# Multi ciclo' > "$project/README.md"
printf '%s\n' '# Plano' > "$project/plan.md"
printf '%s\n' '{}' > "$project/opencode.json"
cp "$ROOT/.opencode/agents/ralph-review.md" "$project/.opencode/agents/ralph-review.md"
printf '%s\n' '{"schema_version":"1.0.0","workflow_id":"wf_multicycle","plan_file":"plan.md","knowledge_policy":{"mode":"non_blocking"},"features":[{"feature_key":"FEATURE-MULTICYCLE","title":"Multiplos ciclos de correcao","position":1}]}' > "$project/workflow.json"

# Fake ralph: gera DOIS resultados de impl COMPLETOS (cycle-1 e cycle-2, como
# acontece quando o bloco passa por correção) + um verify. Este é o cenário do
# finding 7: sem a correção, o controlador contava 2 impls e caía em
# recovery.required.
...
if printf '%s\n' "$*" | grep -q -- '--verify-only'; then
  ...
  # DOIS ciclos de impl COMPLETOS (correção): cycle-1 e cycle-2.
  write_result impl completed 0 "$PWD/.phases/logs/phase-01.cycle-1.log.result.json"
  write_result impl completed 0 "$PWD/.phases/logs/phase-01.cycle-2.log.result.json"
fi
printf '%s\n' 'RALPH_FEEDBACK {"event":"fixture_done","source":"multicycle"}'
SH
...
(cd "$project" && control init --workflow wf_multicycle --manifest workflow.json >/dev/null)
claim="$(cd "$project" && control claim --workflow wf_multicycle --feature FEATURE-MULTICYCLE --actor test)"
...
run_output="$(cd "$project" && \
  ...
  control run --workflow wf_multicycle --feature FEATURE-MULTICYCLE \
    --lease "$lease" --engine opencode --test-cmd true --heartbeat-interval 1)"

printf '%s' "$run_output" | grep -q '"exit_code": 0' || fail 'run multi-ciclo não terminou verde'

# O finding 7: NÃO pode haver recovery.required por "resultado normativo sem
# exatamente um sucesso por modo". Dois impls completed devem ser aceitos.
EVENTS_FILE="$project/.git/ralph-control/events.jsonl" php -r '
    ...
    $counts = ["recovery.required" => 0, "block.finished" => 0, "delegation.completed" => 0];
    $modes = [];
    foreach ($events as $line) {
        ...
        if (in_array($type, ["delegation.completed", "delegation.failed"], true)) {
            $facts = $event["facts"] ?? [];
            $modes[] = ($facts["execution_mode"] ?? "") . ":" . ($facts["status"] ?? "");
        }
    }
    sort($modes);
    if ($counts["recovery.required"] !== 0 || $counts["block.finished"] !== 1 || $counts["delegation.completed"] < 2
        || count(array_filter($modes, static fn (string $m): bool => str_starts_with($m, "impl:"))) < 1
        || count(array_filter($modes, static fn (string $m): bool => str_starts_with($m, "verify:"))) < 1) {
        fwrite(STDERR, json_encode(["counts" => $counts, "modes" => $modes])."\n");
        exit(1);
    }
' || fail 'ledger rejeitou bloco com múltiplos ciclos de impl (recovery.required indevido)'

printf '%s\n' 'OK: bloco com múltiplos ciclos de correção é aceito (2 impls + verify), sem recovery.required.'
