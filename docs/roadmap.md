...
- [x] classificar o resultado do comando de gate: `gate.rejected` (evidência
  ...
  próprios no ledger;
- [x] retry de gate não re-executa bloco já commitado: retoma só do gate
  pendente/rejeitado, sem nova sessão de implementação;
- [x] exigir evidência não-vazia: comando de gate com `exit 0` e stdout vazio
  é tratado como defeito de harness;
- [x] implementar `ralph-control gate --test <gate>`: self-test do comando em
  modo fixture (contrato/evidência/exit) antes de trabalho real;
- [x] documentar no `AGENT_GUIDE` e atualizar CHANGELOG/STATUS.
